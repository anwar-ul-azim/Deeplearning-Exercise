...
########################################
dire = os.getcwd()
TRAIN_DIR = dire + '/data/train'
TEST_DIR = dire + '/data/test'
...
MODEL_SAVE_TO_DISK = False #getenv('KAGGLE_WORKING_DIR') != '/kaggle/working'
...
def fit_model(model, train_generator, val_generator, save=False):
    ...
    history = model.fit_generator(train_generator, epochs=10, steps_per_epoch=100,validation_steps=0.1, validation_data=val_generator)
...
def evaluate_model(generator):
    start_time = time.time()
    evaluations = CNN_MODEL.evaluate_generator(generator,steps=5)
    ...
    start_time = time.time()
    predictions = CNN_MODEL.predict_generator(generator,steps=5)
